// getting website data from browserbase
...
export const analyzeUrl = async (req, res) => {
  try {
    ...
    const aiResult = await analyzeSeoData(scrapeResult.data)
        ...
        if(!aiResult.success){
        analysis.status = "failed";
        analysis.errorMessage = aiResult.error || "Analysis failed. Please try again.";
        await analysis.save()
        return;
        }
...
export const getAnalyses = async (req, res) => {
  try {
    ...
    // True average score across ALL completed analyses (not just this page)
    const avgResult = await Analysis.aggregate([
      { $match: { userId: new mongoose.Types.ObjectId(req.userId), status: "completed" } },
      { $group: { _id: null, avgScore: { $avg: "$overallScore" } } }
    ]);
    const avgScore = avgResult.length ? Math.round(avgResult[0].avgScore) : 0;
    
    res.json({ success: true, analyses, avgScore, pagination: {page, limit, total, pages: Math.ceil
    (total / limit)} });
