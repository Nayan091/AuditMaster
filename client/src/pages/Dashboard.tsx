...
export default function Dashboard() {
    ...
    const fetchRecent = async () => {
        ...
        if(res.data.success){
            ...
            setTotalScans(res.data.pagination?.total ?? res.data.analyses.length)
            setAvgScore(res.data.avgScore ?? 0)
        }
        } catch (err) {
        ...
        <div className="min-h-screen pt-16 md:pt-24 bg-background">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="mb-8 text-center">
                    ...
                    <p className="text-muted-foreground text-l">Track your overall domain health & active audits.</p>
                </div>

                {/* Quick Analyze */}
                {/* <form onSubmit={handleAnalyze} className="mb-10" style={{ animationDelay: "100ms" }}> */}
                <form onSubmit={handleAnalyze} className="mb-10 max-w-2xl mx-auto" style={{ animationDelay: "100ms" }}>
                    <div className="border border-primary/20 rounded-full p-2 flex items-center gap-2 max-w-2xl bg-card">
