...
export default function RankTracker() {
            ...
            const pollInterval = setInterval(async () => {
                try {
                    ...
                    if(check.data.tracking.status !== "checking") {
                        ...
                        setKeywords((prev) => prev.map((k) => (k._id === id ? check.data.tracking 
                        : k)))
                    }
                } catch (error: any) {
                    console.error(error);
    ...
    const handleToggle = async (id: string) => {
        ...
        if(res.data.success){
            setKeywords((prev) => prev.map((k) => (k._id === id ? {...k, active: res.data.
            tracking.active } : k)))
        }
        } catch (err) {
    ...
    }, [keywords.some((k) => k.status === "checking")]);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm mt-1">Monitor your keyword rankings, see top-ranking competitors, and compare your SEO.</p>
                    </div>
