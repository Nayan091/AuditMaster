...
export default function Report() {
    ...
    const fetchAnalysis = async () => {
        ...
        if(res.data.success){
            ...
            return; // don't stop loading yet, still polling
            }
            setAnalysis(res.data.analysis)
            setLoading(false);   // ✅ stop loading once we have final data 
        }else{
            setError("Analysis not found")
            setLoading(false);   // ✅ stop loading on not-found
        }
    }catch{
    ...
    const getCompetitionClass = (label: string) => {
        if (label === "High") return "severity-critical";
        if (label === "Medium") return "severity-warning";
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center bg-card border border-border rounded-2xl p-10">
                    ...
                    <p className="text-muted-foreground text-sm mb-6">{analysis.errorMessage || "The AI model might be down. Please try again later."}</p>
                    ...
                    {activeTab === "keywords" && (
                        <div className="bg-card border border-border rounded-2xl p-6">
                            ...
                            <p className="text-xs text-muted-foreground mb-5">
                                AI-estimated ranking difficulty per keyword, based on general search competitiveness. This is an estimate, not live SERP data.
                            </p>
                                ...
                                <>
                                    ...
                                    <div className="flex items-center gap-3 mb-5 flex-wrap">
                                        <span className="severity-critical px-2.5 py-1 rounded-full text-xs font-semibold">
                                            {analysis.keywords.filter((k) => k.competitionLabel === "High").length} High
                                        </span>
                                        <span className="severity-warning px-2.5 py-1 rounded-full text-xs font-semibold">
                                            {analysis.keywords.filter((k) => k.competitionLabel === "Medium").length} Medium
                                        </span>
                                        <span className="severity-info px-2.5 py-1 rounded-full text-xs font-semibold">
                                            {analysis.keywords.filter((k) => k.competitionLabel === "Low").length} Low
                                        </span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {[...analysis.keywords]
                                            ...
                                            .map((kw) => (
                                                <div key={kw.word} className="flex items-center gap-3 p-3 bg-muted/30 border border-border rounded-xl">
                                                    ...
                                                    <div className="w-24 h-1.5 rounded-full bg-white/5 overflow-hidden hidden sm:block">
                                                        <div className="h-full rounded-full bg-accent" style={{ width: `${kw.competitionScore}%` }} />
                                                    </div>
                                                    <span className="text-xs text-muted-foreground w-8 text-right">{kw.competitionScore}</span>
                                                    <span className={`${getCompetitionClass(kw.competitionLabel)} px-2.5 py-1 rounded-full text-xs font-semibold w-16 text-center`}>{kw.competitionLabel}</span>
                                                </div>
                                            ))}
                                    </div>
                                </>
                            ...
                            {analysis.issues.length > 0 ? (
                                <>
                                    ...
                                    <div className="space-y-3">
                                        {[...analysis.issues]
                                            .sort((a, b) => {
                                                const order = { "Fix First": 0, "Fix Soon": 1, "Nice to Have": 2 };
